...
public class BottomBar extends View {
    ...
    public void build() {
        ...
        if (!iconBitmapAfterList.isEmpty()) {
            //读取传递图片的宽高
            iconHeight = iconBitmapAfterList.get(iconBitmapAfterList.size() - 1).getHeight();
            iconWidth = iconBitmapAfterList.get(iconBitmapAfterList.size() - 1).getWidth();
        }
     ...
     * @return 切换的fragment
     */
    public Fragment switchPage(int page) {
        if (null != fragmentList && !fragmentList.isEmpty() && page < fragmentList.size()) {
            switchFragment(page);
            currentCheckedIndex = page;
            invalidate();
            target = -1;
     ...
     * 获取当前fragment
     *
     * @param page 当前tab的index
     * @return
     */
    public Fragment getCurrentFragment(int page) {
        if (null != fragmentList && !fragmentList.isEmpty() && page < fragmentList.size()) {
            Fragment fragment = fragmentList.get(page);
            return fragment;
        } else {
            return null;
        }
